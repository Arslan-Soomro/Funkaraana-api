...
export const createUserTable = async (drop = false) => {
    try{
        ...
        const constraints = "id INT PRIMARY KEY NOT NULL AUTO_INCREMENT, name VARCHAR(255) NOT NULL, username VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL, password VARCHAR(255) NOT NULL, type VARCHAR(1) NOT NULL DEFAULT 'B'"
        const result = await db.execute(`CREATE TABLE ${tableName}(${constraints})`);
    }catch(err){
...
export const createProductTable = async (drop = false) => {
    try{
        ...
        if(drop) await db.execute(`DROP TABLE IF EXISTS ${tableName}`);
        const constraints = "userID INT NOT NULL, id INT PRIMARY KEY NOT NULL AUTO_INCREMENT, name VARCHAR(255) NOT NULL, description VARCHAR(255) NOT NULL, price INT NOT NULL, image varchar(255) NOT NULL, FOREIGN KEY (userID) REFERENCES users(id)"
        const result = await db.execute(`CREATE TABLE ${tableName}(${constraints})`);
    }catch(err){
...
export const getProductBy = async (byAttr: string, byAttrVal: string | number) => {
...
export const insertIntoProduct = async (data: PRODUCT_DATA) => {    
    ...
    if(!validationRes.error){
        ...
        if(await userExistsById(data.user_id)){
            const insertionQuery = "INSERT INTO products (userID, name, description, price, image) VALUES (?, ?, ?, ?, ?)";
            await db.execute(insertionQuery, [data.user_id, data.name, data.description, data.price, data.image]);
